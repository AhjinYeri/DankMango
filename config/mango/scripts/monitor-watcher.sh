...
#
#  (b) GAME DISPLAY -- never automatic, always asks.
#      The monitor you chose for games to open on. It is stored in the DankMango
#      manifest under .userPrefs.mainDisplay (the KEY keeps its original name --
#      only the words shown to people changed, and renaming a stored key would
#      strand every existing install). If that monitor disconnects we pick a safe
#      temporary stand-in so anything keyed on it keeps working, but we NEVER
#      rewrite your stored choice behind your back -- we send a notification with
#      buttons and let you decide. Replug the real one and everything goes back to
#      normal with no further prompting.
#
...
#      picked a game display -- it isn't a stub, don't "fix" it later.
#
# -----------------------------------------------------------------------------
#  WHERE THINGS ARE
# -----------------------------------------------------------------------------
...
#    effective game display (for consumers to read, see below)
...
#
#  THE EFFECTIVE-GAME-DISPLAY SEAM: the manifest holds what you CHOSE; that
#  runtime file holds what's actually usable RIGHT NOW (they only differ while
#  your chosen monitor is unplugged). Anything that needs "the game display" --
...
#    monitor-watcher.sh --once       # do one tagrule regen + game-display check, exit
...
# CMD: ~/.config/mango/scripts/monitor-watcher.sh --once :: do one tagrule + game-display pass, then exit
...
write_main_rules() {
  ...
  tmp="$(mktemp)" || return 1
  {
    ...
    echo "# DO NOT hand-edit: it gets rewritten whenever the game display changes."
    echo "# Sourced by config.conf:  source-optional=~/.config/mango/dms/mainmonitor.conf"
    echo "#"
    if [ -n "$name" ]; then
      echo "# Game display: $name  -- Steam games open here instead of wherever the"
      ...
      echo "# No game display chosen, so there are no rules here and Steam games open"
...
# -- once you replug the real monitor, "Game display disconnected" has to stop
...
notify_main_display_gone() {
  ...
  fire_notification critical 0 \
    "Game display disconnected" \
    "$gone is gone, so games will open on $standin for now — your saved choice is unchanged. Hover this notification to choose." \
    on_action_setmain "$DRIFT_ID_FILE" \
    "setmain:$standin=Use $standin for games" "keep=Keep $gone"
...
notify_new_monitor() {
  ...
  fire_notification low 10000 \
    ...
    "$name is now connected. Hover this notification to open games on it." \
    on_action_setmain "" \
    "setmain:$name=Open games here"
...
case "${1:-}" in
  ...
  --status)
    ...
    echo "stored game display : $(stored_main_display || true)"
    echo "effective game display : $(cat "$EFFECTIVE_MAIN" 2>/dev/null || echo '(unset)')"
