...
Every DankMango script documents its own flags — `./install.sh --help`, `./update.sh --help`, `./uninstall.sh --help`, and the same for the helper scripts in `~/.config/mango/scripts/`. You don't have to hunt for them: the in-session help hub (`SUPER+SHIFT+/`) has a **Commands you can type** entry: a flat list of every DankMango command, exactly as you would type it, each with a one-line description — read it and copy the line you need. Same list from a terminal with `~/.config/mango/scripts/docs-hub.sh flags`, or `docs-hub.sh flags update.sh` for one script's full `--help`.
